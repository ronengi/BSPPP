...
int main() {
  string i_name = "raw_temps";
  string o_name = "raw_temps.out";
  ...
  // get all temps from all readings
  vector<double> temps;
  for (int y = 0; y < years.size(); ++y) {
    for (int m = 0; m < years[y].months.size(); ++m) {
      for (int r = 0; r < years[y].months[m].readings.size(); ++r) {
	temps.push_back(years[y].months[m].readings[r].temp.temp); // add temp in this reading
      } // readings
    } // months
  } // years
